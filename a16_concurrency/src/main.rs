...
// 由于不需要语言提供并发相关的基础设施，并发方案不受标准库或语言所限：
// 我们可以编写自己的或使用别人编写的并发功能。
...
// The Rustonomicon 中有更多关于这些保证以及如何维持他们的信息。
